...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate zero-shot SAM ViT-B on the Kvasir-SEG test split."
    )
    ...
    parser.add_argument(
        ...
        help="Path to the SAM ViT-B checkpoint.",
    )
...
def main():
    ...
    visualization_dir = output_dir / "zero_shot_visualizations"
    ...
    for index, (image_path, mask_path) in enumerate(pairs, start=1):
        ...
        if index % 10 == 0 or index == len(pairs):
            print(f"Processed {index}/{len(pairs)} images")

    if not rows:
        raise RuntimeError("No images were evaluated. Check masks and dataset paths.")

    csv_path = output_dir / "zero_shot_baseline.csv"
    summary_path = output_dir / "zero_shot_summary.txt"
